...
node_t  *enqueue(node_t **head , int key , char *string )
{
...
{   int retVal ;
    node_t *temp ;
    node_t *current = *head;
    current = current->next; // lo porto al nodo che devo cancellare
    temp = current;  // memorizzo temp su cui viene fatta la free
    current = current->next; // sposto al nodo successivo current
    (*head)->next = current ; // collego la testa a current
    current->prev= (*head);  // collego current->prev alla testa
    retVal = temp->key;
    free(temp);     // isolato temp ne faccio la free
    return retVal;
}
...
void top(node_t **head)
{
    printf(" la cima della lista e : %d \n" , (*head)->next->key);
}

// e un eliminazione in coda
int pop(node_t **head)
{   int retVal;
    node_t *current ;
    current  = *head ; // punto alla testa

// scorro fino al penultimo nodo ,  ne creo uno alias per avanzare sul mio target e poi ricollgo il penultimo nodo co null e l ultimo che punto con temp lo cancello
...
//e un inserimento in coda
node_t *push(node_t **head , int key , char *string )
{

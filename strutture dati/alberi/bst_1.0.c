...
// funzione che mi dice se sono su una foglia
bool isLeaf(node_t *T, int data)
{
     node_t *nodo = search(&T, data);

        if(nodo->leftChild ==NULL || nodo ->rightChild ==NULL){
            printf("il nodo e una foglia ");
            return true;
        }else{
            printf("il nodo NON e una foglia ");
            return false
        }
}

bool isRoot(node *T , int data)
{
    if
...
// procedure di eliminazione albero
void free_element(node_t *T , int item)
{
;
}

void free_Tree(node_t *T , int item)
{
;
}

...
// funzione che mi ritorna il puntatore al nodo con valore massimo
struct node *massimo(struct node *head , struct node *max )
{
    if(head == NULL)
        return max;
...
// mi torna il puntatore alla testa
struct node *Pos_head(struct node *list_head)
{
    return(list_head);
}

// mi ritorna il puntatore in coda
...
/*
 quest procedura ci dice che questo e un controllo su la lista
 1) se la lsirta e vuota
 2) il numero di nodi
 3) l indirizzo di testa
 4) l indirizzo di coda
...
// algoritmo di inserimento di un nuodo qualsiasi sortato
...
int main(void)
{
    ...
    control(&list_head);
    printf("\n");
    insert_head(&list_head, 0);
    insert_sorted(&list_head, 0);
    insert_tail(&list_head, 0);
     insert_tail(&list_head, 0);
    insert_sorted(&list_head , 0);
    insert_sorted(&list_head , 0);
    insert_sorted(&list_head , 0);
    ...
    insert_sorted(&list_head , 0);
    insert_sorted(&list_head , 0);
    insert_sorted(&list_head , 46);
    insert_sorted(&list_head , 0);
    struct node  *max = list_head;
    printf("IL MASSIMO DELLA LISTA E : %d \n",  (massimo(list_head, max ))->data );
    printf("\n");

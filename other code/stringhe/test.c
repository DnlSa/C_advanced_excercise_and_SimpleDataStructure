...
int childProcess(int i , char * filename){
    ...
    char buff[MAX_SIZE];
    char *qualc = addr; // addr lo teniamo sempre come indirizzo base
    while ((byte_r = read(fd , (char *)qualc, 1)) != 0){
        qualc++;
        count+=byte_r;
        buff[count] = *qualc;
    ...
    //qualc = strcpy(buff , (char *)addr);
    memcpy(buff, (char *)addr , strlen((char *)addr));

    printf("%ld , \n%s \n",strlen((char *)addr) , buff);

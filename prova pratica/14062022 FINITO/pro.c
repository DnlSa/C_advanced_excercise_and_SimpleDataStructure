...
/*
 gcc prog.c -o prog
 ./pro -f primo.txt secondo.txt terzo.txt quarto.txt quinto.txt  -s casa erba meridiana cancello apostrofo
...
int main(int argc , char **argv){
	...
	int i;
	n_arg = (argc-3)/2; // codifica degli argomenti dove ho fatto cosi (argomenti totali - nome_prog - (-f) - (-s)
	printf("%d\n", n_arg);
	int j;
	char *filename[n_arg];
	char *string[n_arg];
	...
	for(k=0 , i=2; k<n_arg ;k++,i++){ // k e il contatore argomenti
		filename[k]=argv[i]; // questo mappa i file
		string[k] = argv[i+(n_arg+1)]; // i+(n_arg+1) mappa le stringhe
	}
		for(i=0 ; i<n_arg;i++){
			printf("%s , %s \n", filename[i], string[i]);
		}
	...
	sem_t *sem[n_arg+1]; // semafori con n_arg+1 cioe se n_arg sono 5 allora avremo semaforo 0,1,2,3,4 il piu 1 include anche il quinto semaforo
	for(i=0 ; i<n_arg;i++){ //questi sono semafori da 0 a n
		sem[i]=(sem_t *)mmap(NULL , sizeof(sem_t), PROT_READ|PROT_WRITE , MAP_SHARED|MAP_ANONYMOUS , 0,0 );
	}

	sem[n_arg]=(sem_t *)mmap(NULL , sizeof(sem_t), PROT_READ|PROT_WRITE , MAP_SHARED|MAP_ANONYMOUS , 0,0 ); //questo e il quinto semaforo

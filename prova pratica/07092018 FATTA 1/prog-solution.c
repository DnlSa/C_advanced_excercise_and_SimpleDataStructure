...
////////////////////// parte della memoria ////////////////////////////
	...
	//////////////////////////SEMFORI SCRITTORI//////////////////////////////////
		...
		// semafori scrittori
	sd1 = semget(IPC_PRIVATE,num_threads,IPC_CREAT|0666); // instanzia 5 semafori con indirizzo IPC_PRIVATE(l indirizzo e ereditabile dai figli )
	if (sd1 == -1){
	...
	for ( i = 0; i < num_threads; i++) {
		ret = semctl(sd1,i,SETVAL,1);  //  serve a impostare i semafori a 1
		if(ret == -1){
...
//////////////////////SEMAFORI LETTORI/////////////////////////////////////////
	...
	sd2 = semget(IPC_PRIVATE,num_threads,IPC_CREAT|0666); // stessa cosa con semafori lettori
	if (sd2 == -1){
	...
	for ( i = 0; i < num_threads; i++) {
		ret = semctl(sd2,i,SETVAL,0);  // IMPOSTATI TUTTI A 0
		if(ret == -1){
...
void *parent_worker(void *arg) {
	...
	oper.sem_num = me; // il semaforo
	oper.sem_flg = 0; //

	while(1){

		oper.sem_op = -1; // prende un token
redo1:
